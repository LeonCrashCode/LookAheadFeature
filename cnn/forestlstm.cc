...
ForestLSTMBuilder::ForestLSTMBuilder(unsigned N,
                         unsigned layers,
                         unsigned input_dim,
                         unsigned hidden_dim,
...
void ForestLSTMBuilder::new_graph_impl(ComputationGraph& cg){
  param_vars.clear();
  for (unsigned i = 0; i < layers; ++i){
    auto& p = params[i];
...
// layout: 0..layers = c
//         layers+1..2*layers = h
void ForestLSTMBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
  h.clear();
  c.clear();
  if (hinit.size() > 0) {
...
Expression ForestLSTMBuilder::add_input(const vector< vector<int> >& childrens, const Expression& x) {
...
Expression ForestLSTMBuilder::add_input_impl(int prev, const Expression& x) {
  assert (false);
  return x;
}

void ForestLSTMBuilder::copy(const RNNBuilder & rnn) {
  const ForestLSTMBuilder & rnn_treelstm = (const ForestLSTMBuilder&)rnn;
