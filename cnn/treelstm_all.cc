...
TreeLSTMAllBuilder::TreeLSTMAllBuilder(unsigned N,
                         unsigned layers,
                         unsigned input_dim,
                         unsigned hidden_dim,
...
void TreeLSTMAllBuilder::new_graph_impl(ComputationGraph& cg){
  param_vars.clear();
  for (unsigned i = 0; i < layers; ++i){
    auto& p = params[i];
    auto& lp = lparams[i];
...
// layout: 0..layers = c
//         layers+1..2*layers = h
void TreeLSTMAllBuilder::start_new_sequence_impl(const vector<Expression>& hinit) {
  h.clear();
  c.clear();
  if (hinit.size() > 0) {
...
Expression TreeLSTMAllBuilder::add_input(const vector< vector<int> >& childrens, const Expression& x) {
...
Expression TreeLSTMAllBuilder::add_input_impl(int prev, const Expression& x) {
  assert (false);
  return x;
}

void TreeLSTMAllBuilder::copy(const RNNBuilder & rnn) {
  const TreeLSTMAllBuilder & rnn_treelstm = (const TreeLSTMAllBuilder&)rnn;
  ...
  assert(wparams.size() == rnn_treelstm.wparams.size());
