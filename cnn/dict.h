...
public:
  ...
  Dict(bool _map_unk, int _unk_id): frozen(false), map_unk(_map_unk), unk_id(_unk_id) {
  }
...
//void normalize_digital_lower(std::string& word);
//void ReadSentenceMultilabel(const std::string& line, 
//			    Example_item* s, Dict* sd, Dict* cd, Dict* td, Dict* ld, int* unks, int* unkc, int* unkt);
